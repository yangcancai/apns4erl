...
%% API
-export([start_link/2, default_connection/2, name/1, host/1, port/1, certdata/1,
         certfile/1, keydata/1, keyfile/1, type/1, gun_pid/1, close_connection/1,
         push_notification/4, push_notification/5, cast_push_notification/4,
         wait_apns_connection_up/1]).
%% gen_statem callbacks
-export([init/1, callback_mode/0, open_connection/3, open_origin/3, open_proxy/3,
         open_common/3, await_up/3, proxy_connect_to_origin/3, await_tunnel_up/3, connected/3,
         down/3, code_change/4]).

-export_type([name/0, host/0, port/0, path/0, connection/0, notification/0, type/0]).
...
-type keydata() ::
    {'RSAPrivateKey' | 'DSAPrivateKey' | 'ECPrivateKey' | 'PrivateKeyInfo', binary()}.
-type proxy_info() ::
    #{type := connect,
      host := host(),
...
%% @doc Pushes notification to certificate APNs connection.
-spec cast_push_notification(name() | pid(),
                             apns:device_id(),
                             notification(),
                             apns:headers()) ->
                                reference().
...
push_notification(ConnectionId, Token, DeviceId, Notification, Headers) ->
    gen_statem:call(ConnectionId,
                    {push_notification, Token, DeviceId, Notification, Headers}).
...
init({Connection, Client}) ->
    ...
    {ok, open_connection, StateData, {next_event, internal, init}}.

-spec open_connection(_, _, _) -> _.
open_connection(internal, _, #{connection := Connection} = StateData) ->
    NextState =
        case proxy(Connection) of
            #{type := connect} ->
                open_proxy;
            undefined ->
                open_origin
        end,
    {next_state, NextState, StateData, {next_event, internal, init}}.
...
await_up(info, {gun_up, GunPid, Protocol}, #{gun_pid := GunPid} = StateData) ->
    ...
    NextState =
        case proxy(Connection) of
            #{type := connect} when Protocol =:= http ->
                proxy_connect_to_origin;
            undefined when Protocol =:= http2 ->
                connected
        end,
    {next_state, NextState, StateData, {next_event, internal, on_connect}};
...
proxy_connect_to_origin(internal, on_connect, StateData) ->
    ...
    {next_state,
     await_tunnel_up,
     StateData#{gun_connect_ref => ConnectRef},
...
await_tunnel_up(info,
    ...
    {next_state, connected, StateData, {next_event, internal, on_connect}};
...
connected(cast,
    ...
    StreamRef = push_cast(GunPid, DeviceId, Headers, Notification),
    {next_state, connected, StateData#{StreamRef => Req}};
...
down(state_timeout, backoff, StateData) ->
    {next_state, open_connection, StateData, {next_event, internal, init}};
...
-spec push_cast(pid(), apns:device_id(), apns:headers(), notification()) ->
                   apns:stream_id().
push_cast(GunConn, DeviceId, HeadersMap, Notification) ->
    Headers = get_headers(HeadersMap),
    Path = get_device_path(DeviceId),
    gun:post(GunConn, Path, Headers, Notification, #{reply_to => self()}).
...
gun_loop({gun_data, _ConnPid, StreamRef, nofin, Data}, State) ->
